          ...
          <div key={order.id} className="order-card">
              ...
              <div key={item.productId} className="order-product-box">
                <div className="order-product-img-wrapper">
                  ...
                  {order.status !== 'DELIVERED' && order.status !== 'CANCELLED' && (
                    <button className="order-review" style={{ marginTop: '8px', width: '100%' }}>Đánh Giá</button>
                  )}
                </div>

                <div className="order-product-info" onClick={() => navigate(`/product/${item.productId}`)}>
                  <h3 className="order-product-name">{item.productName}</h3>
                  <p className="order-product-variant">Loại hàng: {item.productVariant}</p>
                  <p className="order-qty">x{item.quantity}</p>
                </div>

                <div className="order-price-box">
                  <span className="order-old-price">{item.originalPrice?.toLocaleString('vi-VN') + '₫'}</span>
                  <span className="order-current-price">{item.finalPrice?.toLocaleString('vi-VN') + '₫'}</span>
                </div>
              </div>
            ))}
            ...
            <div className='order-info'>
              <div className="order-info-box">
                <p className="order-shipping-address">Địa chỉ: {order.shippingAddress}</p>
                <p className="order-payment-method">Phương thức thanh toán: {order.paymentMethod}</p>
              </div>

              <div className="order-total-box">
                <span className="order-total-label">Thành tiền:</span>
                <span className="order-total">{order.totalPrice?.toLocaleString('vi-VN') + '₫'}</span>
              </div>
            </div>
